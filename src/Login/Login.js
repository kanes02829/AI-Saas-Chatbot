...
class Login extends Component {
  ...
  onLogin = async (e) => {
    ...
    try {
      let data = [];
      if (id) {
        data = await this.props.store.api
          .post('/auth/signin/' + id.toString(), {
            email: this.email,
        ...
        //   const toastId = this.props.toast.addToast(data.message, {
        //     appearance: 'error',
        //     autoDismiss: false,
        //   });
        //   setTimeout(() => {
        //     this.props.toast.removeToast(toastId);
        //   }, 4000); // Remove the toast after 4 seconds
        //   return;
        // }
      } else {
        data = await this.props.store.api
      ...
      const toastId = this.props.toast.addToast(messages.successLogin, {
        appearance: 'success',
        autoDismiss: false,
      });
      setTimeout(() => {
  ...
  onSignup = async (e) => {
    try {
      e.preventDefault();
      this.errorMessage = '';
      console.log('hey:', this.props.store.referral);
      ...
      const toastId = this.props.toast.addToast(messages.successSignUp, {
        appearance: 'success',
        autoDismiss: false,
      });
      setTimeout(() => {
  ...
  // 939151310813-4f48mrqtkflefcsh0p9qd7tufofijqtq.apps.googleusercontent.com
  render() {
    const { isLoggedIn, user } = this.state;
    console.log('location: ', this.props.location.pathname.slice(0, 8));
    return (
      <>
        <GoogleOAuthProvider clientId="1032830281470-2bji92ue7o711shbegcgd0en4k50la5i.apps.googleusercontent.com">
          <GauthComp
        ...
        <div className="container mx-auto lg:px-4 py-4 min-h-screen flex flex-col md:items-center md:justify-center">
          <div className="text-center mb-6">
            ...
            <div className="text-3xl md:text-5xl relative font-medium text-transparent bg-clip-text bg-gradient-to-r from-gray-700 to-gray-600 mb-4">
              AI<span className="font-normal "> Assistant</span>
...
const GauthComp = ({ store, toast, errorMessage }) => {
      ...
      axios
        ...
        .then(async (res) => {
          ...
          try {
            ...
            const toastId = toast.addToast(messages.successSignUp, {
              appearance: 'success',
              autoDismiss: false,
            });
            ...
            if (
              ...
              try {
                ...
                console.log(data);
                store.loginWithDataTokenAndProfile(data);
                const toastId = toast.addToast(messages.successLogin, {
                  appearance: 'success',
                  autoDismiss: false,
                });
                setTimeout(() => {
                ...
                if (err?.response?.data?.message) {
                  ...
                  const toastId = toast.addToast(err?.response?.data?.message, {
                    appearance: 'error',
                    autoDismiss: false,
                  });
                  setTimeout(() => {
