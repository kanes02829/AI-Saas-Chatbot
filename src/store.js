...
class appStore {
  ...
  init = async () => {
    try {
      ...
      const currentOrgInfo = localStorage.getItem('currentOrgInfo');

      if (profile && token && currentOrgInfo) {
        this.api.defaults.headers.common['x-access-token'] = token;
        this.profile = JSON.parse(profile);
        this.currentOrgInfo = JSON.parse(currentOrgInfo);
  ...
  loginWithDataTokenAndProfile = async (data) => {
    ...
    this.setCurrentOrgInfo(data.currentOrgInfo);
    // this.currentOrgInfo = data.currentOrgInfo;
    this.isLoggedIn = true;
  };
  ...
  setCurrentOrgInfo = async (currentOrgInfo) => {
    this.currentOrgInfo = currentOrgInfo;
    localStorage.setItem('currentOrgInfo', JSON.stringify(currentOrgInfo));
  };
  ...
  handleLogout = () => {
    ...
    localStorage.removeItem('currentOrgInfo');
  };
